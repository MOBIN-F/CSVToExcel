...
public class CSVToExcel {
    ...
    public void convert(String originPath,String targetPath){
        try(InputStream input = new FileInputStream(originPath);//读取源转换文件如:f:\\test
            Reader reader = new InputStreamReader(input,"UTF-8");
            BufferedReader bufferedReader = new BufferedReader(reader);

            FileOutputStream out = new FileOutputStream(targetPath)){  //输出文件如：:f:\\test.xls
            ...
            for(String data = bufferedReader.readLine(); data != null; data = bufferedReader.readLine()){
                ...
                Integer lineRow = mapRow.get(value_0[0]);   //获取该值所对应的行，如果为空表示是新行
                Integer lineCol = mapCol.get(value_1[0]);   //获取表头所对应的列，如果为空表示是新表头字段
                if((lineRow != null)){//同一行
                    Row curr_row= sheet.getRow(lineRow);
                    isaddHeader(sheet,lineCol,cellnum_1,value_1[0]);  //是否添加表头
                    curr_row.createCell(cellnum_1).setCellValue(str[2]);
                }else {//不同行
                    ++lineNum;
                    mapRow.put(value_0[0],lineNum);
                    Row row = sheet.createRow(lineNum);
                    row.createCell(0).setCellValue(value_0[0]);  //第一列值
                    isaddHeader(sheet,lineCol,cellnum_1,value_1[0]);  //是否添加表头
    ...
    public void isaddHeader(Sheet sheet,Integer lineCol,Integer cellnum_1,String value_1){
